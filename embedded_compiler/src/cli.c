...
static int cli_generate_outputs(cli_context_t *context) {
  ...
  LOG_TRACE("cli_generate_outputs output_dir=%s html_bytes=%zu css_bytes=%zu\n",
            context->output_dir,
  ...
  if (generator_validate_embedded_bundle(context->output_dir) != 0) {
    return 1;
  }
...
int cli_run(int argc, char **argv) {
  ...
  log_printf("Processed %zu files, %zu tokens, %zu validated summaries.\n",
             ...
             context.token_count,
             context.validated_file_count);
  log_printf("Generated embedded bundle in %s\n", context.output_dir);
  cli_free_context_buffers(&context);
  log_close();
  return 0;
}
