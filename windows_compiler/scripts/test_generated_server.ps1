...
try {
  ...
  [pscustomobject]@{
    root_ok = ($root.StatusCode -eq 200 -and -not $rootHasRawTemplate -and $rootHasExpectedTabs -and $rootHasExpectedViews -and $rootHasAssets)
    ...
    js_ok = ($js.StatusCode -eq 200 -and $js.Content -match 'fetchJson' -and $js.Content -match 'showView' -and $js.Content -match "window\.history\.replaceState")
